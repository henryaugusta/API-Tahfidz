...
$error = false;
if (!isset($_POST['mentor_id']) || empty($_FILES['uploaded_files']) || $_POST['mentor_id'] == null) {
    $error = true;
}
...
if ($error) {
    ...
    $teacherID = $_POST['mentor_id'];
    $sql = mysqli_query($conn, "SELECT *  from mentor where id='$teacherID' ");
    if ( !$sql ) {
        echo mysqli_error($conn);
    }
    $row = mysqli_fetch_array($sql);
    $teacherContact = $row['contact'];

    $file_storeName = "./photo/$teacherContact" . ".png";
    $uploadStat =  move_uploaded_file($file_temp_loc, $file_storeName);

    if ($uploadStat) {
        $query = "UPDATE mentor SET url_profile='$file_storeName', updated_at='$nowDate' where id ='$teacherID'";
